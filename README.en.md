...
The Web application bundle loads this package as one Cordis plugin. 小深 and 阿良 work without Codex Desktop; an unset selection resolves to 小深. Open **Settings → Pets** to enable or hide the overlay, select another available preset or imported pet, choose a width from 80 to 224 CSS pixels, or refresh the catalog after changing files under the Codex home. The overlay stays inside the browser viewport; pointer dragging and arrow keys reposition it for the current page.
...
Platform discovery checks `/Applications/ChatGPT.app/Contents/Resources/app.asar` on macOS and `%LOCALAPPDATA%\Programs\ChatGPT\resources\app.asar` on Windows. Other platforms need `appAsarPath` for Codex presets, but 小深, 阿良, and custom Codex-home packages remain available without it.
...
| `selectedId` | `dsh` | Selects 小深 (`dsh`), optional bundled 阿良 (`aliang`), a Codex preset id, or the opaque `custom:<directory>` id assigned during discovery. |
