...
//#region lib/types/codex-asar.js
...
const MAX_ASAR_HEADER_BYTES = 32 * 1024 * 1024;
...
//#endregion
//#region lib/types/codex-format.js
...
const MAX_PET_ASSET_BYTES = 20 * 1024 * 1024;
...
async function readOnePet(petsRoot, directoryName, manifestName) {
	const directoryPath = await realpath(join(petsRoot, directoryName));
...
//#endregion
//#region lib/types/pet-catalog.js
...
*/
async function readBundledPetAsset(source) {
	const body = await readFile(source);
	if (body.byteLength > 20971520) throw new Error("bundled DSH pet atlas exceeds the size limit");
...
function resolveCodexHome(configured, env = process.env) {
	...
	return resolve(expandHomePath(configuredPath || environmentPath || join(homedir(), ".codex")));
}
...
var PetCatalog = class {
	...
	*/
	...
	async scanAndPublish() {
		...
		const builtinAssets = new Map([[DSH_BUILTIN_PET.id, dshBuiltinAsset], ...codexBuiltinAssets]);
...
async function apply(ctx, config = {}) {
	ctx.settings.register(settingsNamespace(PET_SETTINGS_NAMESPACE), PetSettingsSchema);
...
//#endregion
export { CODEX_BUILTIN_PETS, CODEX_PET_ATLASES, CODEX_PET_ATLAS_V1, CODEX_PET_ATLAS_V2, Config, DEFAULT_PET_SETTINGS, DEFAULT_PET_SIZE, DSH_BUILTIN_PET, MAX_PET_SIZE, MIN_PET_SIZE, PET_ASSET_PREFIX, PET_CATALOG_ENDPOINT, PET_HTTP_PREFIX, PET_PRESETS, PET_REFRESH_ENDPOINT, PET_SETTINGS_NAMESPACE, apply, inject, name, petAssetPath, petAssetUrl };
